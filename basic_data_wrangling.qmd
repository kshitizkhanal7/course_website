...
A typical tabular data contains various types of numbers (integers, decimals), character string (e.g. for places, names, etc.), logical values (True or False), etc. These are called data types. 
...
Some good practices for column names include:
...
We can create a new column of Month using the date_2020 column values. Notice the sequence of operations using "|>" operator. The operator is called pipe, and this sequence using pipes is called chaining.
...
Exercise: Use mutate() to create a new column called temp_c that is value of temperature in degree Celsius. Note: the values in temperature column are in Fahrenheit.
